...
public class BinaryWireNumbersTest extends WireTestCommon {
    ...
    private void test(@NotNull WriteValue expected, @NotNull WriteValue perform) {
        ...
        assertEquals(len, bytes1.readRemaining(),
                "Length for fixed length doesn't match for " + TextWire.asText(wire1));
        ...
        assertEquals(bytes1.readRemaining(), bytes2.readRemaining(),
                "Lengths for variable length expected " + bytes1
