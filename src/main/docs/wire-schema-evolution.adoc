...
== Collection, EnumSet and Map Fields: Absent, Null and Empty Input

`WireMarshaller` reads `Collection`, `EnumSet` and `Map` fields the same way, whatever the element type.
The _declared default_ is the value the field holds after the no-argument constructor, so it may be null, empty or populated.
`overwrite` is the flag passed to `Wires.readMarshallable(Object, WireIn, boolean)`.
`Marshallable.readMarshallable` passes `true` and `AbstractMarshallableCfg` passes `false`.

[cols="2,3,3",options="header"]
|===
| Input field | `overwrite=false` | `overwrite=true`
| Absent | Keeps the destination's current value | Restores the declared default
| Present and null | Restores the declared default | Sets null
| Present and empty | Sets empty | Sets empty
| Present and populated | Replaces the contents with the input | Replaces the contents with the input
|===

A restored container is a separate instance from the defaults and from every other destination.
Its elements, keys and values are copied by reference, so a mutable element is still shared with the defaults.
A reused destination keeps nothing from an earlier read, other than in the absent, `overwrite=false` case above.

NOTE: `YamlWire` (`WireType.YAML_ONLY`) reads in input order.
For an ordinary DTO it resets an absent field to its declared default even when `overwrite` is `false`.
A class that overrides `unexpectedField`, which includes every `AbstractMarshallableCfg`, keeps the destination's value as the table says.
...
*Collections :: A `Collection`, `EnumSet` or `Map` field that is missing in the input stream follows the same rule as any other field.
It is restored to its declared default, so a field initialised inline to an empty collection is empty after the read,
and a field with no initialiser is null.
See <<container-field-input>> for explicit nulls and for reads with `overwrite` disabled.
