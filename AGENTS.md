...
* Use `//!` on every logical change to an existing release-code file while that change is under review. Build metadata and release documentation are release code for this purpose; use the corresponding comment syntax, for example `<!-- //! ... -->` in XML.
...
* Do not remove, shorten to an ordinary comment, or omit `//!` notes during cleanup, simplification, squashing, or restacking. Remove a note only when its code is removed or when maintainers deliberately retire it after the post-merge review period.
