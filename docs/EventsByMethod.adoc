...
* Direct method calls remain the simplest form of communication when the producer and consumer are in the same JVM.
* Interfaces describing events become easy to serialise as data structures for persistence or testing.
...
Handler implementation :: A class that implements the event interface to process events locally.
`MethodWriter` :: Generates a proxy that serialises method calls onto a `Wire`.
`MethodReader` :: Reads method calls from a `Wire` and invokes them on a handler.
Wire format :: YAML, JSON or binary encodings used to store or transmit the events.
...
----
graph LR
    A(Event Sources) --> B(Microservice<br/>Method Implementations)
    B --> C(Event Store<br/>Chronicle Queue)
----
...
graph LR
    C(Client) --> G(Gateway)
    S(Internal Systems<br/>sources) --> G
    G --> CR(Client replies)
    G --> DS(Internal Systems<br/>downstream events)
----

=== Testing using YAML

In both cases, these components can be tested using events stored in YAML files.
Events appear as method names with their arguments, which may be simple scalars, a list of values or a DTO represented as a mapping.
