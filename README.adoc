...
===  YAML 2 Features currently suppored in Chronicle-Wire

The following list the features are from the YAML 2 spec ( see http://yaml.org/spec/1.2/spec.html )
which are currently supported by Chronicle-wire, the numbering refers to the section in the spec.
See the test case net.openhft.chronicle.wire.YamlSpec for more details.

the sections below list those areas of the spec that are implemented in Chronicle-Wire
|===============
| Feature                     | Strict Yaml 2 Spec Compliant | Equivalent Operation in
Chronicle-Wire
...
== Yaml Specification Equivalent Operation in Chronicle-Wire
