...
start of the message that an appender writes to buffer. The "len" field includes: a) "complete" flag that
shows whether the appender has completed writing this message or not. b) "metadata" flag that shows whether this
data is metadata and c) the remaining 30 bits that contains the actual size of data. After completing writing the message, the
appender returns to the "len" field and sets the "complete" flag to one so that it indicates the message was written
...
image::buffer.jpg[buffer,700,700]
